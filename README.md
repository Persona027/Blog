...
## 更新日志 (2026-02-11)
- **UI 升级**: 文章详情页重构为左中右三栏布局，增强了沉浸式阅读体验。
- **功能修复**: 修复了文章详情页背景遮挡问题，并实现了高性能的目录自动追踪高亮算法。
- **部署维护**: 解决了构建过程中的 TypeScript 类型安全问题，确保 CI/CD 流程畅通。

## 更新日志 (2026-02-12)
- **文档新增**: 收录了 Unity 渲染基础教程，涵盖材质 (Material)、着色器 (Shader)、纹理 (Texture) 以及光照 (Lighting) 模型。
- **内容优化**: 将原始笔记转化为结构化的 Markdown 文章（`unity-render-basics.md`），并归类至 `Unity` 组下。
- **教学互动**: 针对渲染管线、UV 映射及光源属性等进阶概念增加了“导师补充”模块。
- **结构同步**: 调整目录标题层级，确保 3 栏布局下的导航索引自动生成并精准跳转。
- **功能新增**: 实现了文章详情页的三栏式精美布局，包含左侧分类文章列表与右侧自动跟踪目录（TOC）。
- **交互优化**: 引入了滑动高亮目录反馈、平滑滚动锚点定位。
- **响应式设计**: 为移动端设计了双向抽屉式导航，通过顶部操作栏轻松呼出侧边栏与目录。
- **排版增强**: 采用 `prose-invert` 深度定制文章排版，增加了代码高亮、引用块美化及标题自动 ID 注入。
- **技术实现**: 使用 `IntersectionObserver` 替代传统滚动监听，性能更佳且同步刷新 active 状态。
