...
agraph{冗余位计算 [p.82]}：设数据 $m$ 位，冗余位 $r$ 位，则：
$$
...
agraph{冗余位定位 [p.83--84]}：冗余位放在 $2^n$ 位置（第1, 2, 4, 8位），每个 $r$ 位是一组数据位的奇偶校验码：
...
agraph{检错与纠错 [p.88]}：接收端重新计算奇偶校验码，按 $r_4 r_3 r_2 r_1$ 顺序组成二进制数，该数值即为出错位位置。将该位反转即可纠错。
